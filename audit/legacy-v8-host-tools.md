...
| `args = [script_path, cpu_arg, ]` | 8.6.125 | 11.9.169.4 | 233 | 29 |

For V8 before 9.0, every exact tag must match exactly one `vcvarsall` argument template so the installed MSVC v142 toolset can be selected.
