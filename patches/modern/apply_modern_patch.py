...
{backref_line}"""
    ...
    readonly_replacement = "\n".join(
        (
            ...
            "    auto in_read_only_page = [&](Address candidate) {",
            "      for (const ReadOnlyPage* mapped_page : read_only_space->pages()) {",
            "        const Address page_start = mapped_page->ChunkAddress();",
            "        const Address page_end = page_start + mapped_page->size();",
            "        if (candidate >= page_start &&",
            "            candidate <= page_end - kTaggedSize) {",
            "          return true;",
            "        }",
            "      }",
            "      return false;",
